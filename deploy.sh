...
zip $ZIPFILE lambda_handler.js

# Handler = Filename.js + FN-Name
# handler lambda_handler.call

if ! aws lambda get-function --function-name $FNAME 1> /dev/null 2>&1;  then
    echo "Create Function $FNMAE..."
    aws lambda create-function      --function-name $FNAME --zip-file fileb://./$ZIPFILE --runtime nodejs8.10 --role arn:aws:iam::854014537498:role/lambda_basic_execution --handler lambda_handler.call
else
